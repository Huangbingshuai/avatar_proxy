...
- 方舟模型只有在官方仍可调用、当前渠道可访问且完成端到端验证后才能加入内置目录；供应商枚举接口返回模型 ID 不等于推理接口可用。
- 未经验证或确认无法调用且没有历史数据的模型应物理删除，不要用禁用占位继续保留；存在任务、用量或冻结账单时必须先保护历史数据并制定迁移方案。
- 翻译等专用模型必须按实际能力限制 Chat、Responses 和流式模式；客户工具不得把所有文本模型一律按流式 Chat 调用。
- 真实供应商验证会产生费用，只能在用户明确授权后手动执行；自动化测试仍必须使用 MockTransport。
...
- 方舟模型目录、固定上游 ID、能力或验收方法变化：更新 `backend/VOLCENGINE_MODEL_CATALOG.md`，公开客户文档只列当前支持模型。
