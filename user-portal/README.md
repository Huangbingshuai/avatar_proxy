...
模型测试直接使用当前登录的业务 Key 调用 `/v1/*`。项目启用模型后，项目下所有有效业务 Key 自动可用。文本模型默认使用 SSE 流式输出并支持手动停止，图片和视频模型保持普通请求或异步任务模式；参考图输入会按照服务端返回的模型能力显示。模型列表以服务端返回结果为准，测试请求会产生真实供应商用量；页面不会展示或额外持久化完整 Key。
