...
const ACTIVE_VIDEO_STATUSES = new Set(["queued", "pending", "running", "processing"]);
...
export default function ModelPlayground({ apiKey, apiKeyValid }: { apiKey: string; apiKeyValid: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (disposed) return;
        setModels(items);
        setModelId((current) => items.some((item) => item.id === current) ? current : items[0]?.id || "");
      } catch (caught) {
        if (!disposed) setError(caught instanceof Error ? caught.message : "模型列表加载失败");
      } finally {
  ...
  async function runTest() {
    ...
    if (prompt.trim().length < 2) return setError("请输入至少两个字的测试提示词");
    if (
    ...
    try {
      ...
      setResult({ ...response, elapsedMs: Math.round(performance.now() - startedAt), modality: model.modality });
    } catch (caught) {
  ...
  async function refreshVideo() {
    ...
    try {
      ...
      setResult({ ...response, elapsedMs: Math.round(performance.now() - startedAt), modality: "video" });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "视频任务状态刷新失败");
    } finally {
      setRefreshing(false);
    }
  }
  ...
  if (!apiKeyValid) {
    return <section className="officialSection modelLabSection"><div className="officialEmpty"><KeyRound size={30} /><b>请先连接服务</b><p>登录后即可用当前业务 Key 测试项目已启用的文本、图片和视频模型。</p></div></section>;
  }
  ...
  const videoSupportsImage = selectedModel?.modality === "video"
  ...
  return <section className="officialSection modelLabSection" aria-labelledby="model-lab-title">
    <div className="sectionTitleRow modelLabHeader"><div><span className="modelLabEyebrow"><Sparkles size={13} />MODEL LAB</span><h1 id="model-lab-title">模型在线测试</h1><p>使用当前登录的业务 Key 发起真实中转请求，测试结果会计入该 Key 的实际用量。</p></div><button type="button" className="secondaryButton" disabled={loadingModels} onClick={() => {
      setLoadingModels(true);
      setError("");
      void listRelayModels(apiKey).then((items) => {
        setModels(items);
        setModelId((current) => items.some((item) => item.id === current) ? current : items[0]?.id || "");
      }).catch((caught) => setError(caught instanceof Error ? caught.message : "模型列表加载失败")).finally(() => setLoadingModels(false));
    }}>{loadingModels ? <LoaderCircle size={15} className="spin" /> : <RefreshCw size={15} />}刷新模型</button></div>

    {error ? <div className="modelLabError" role="alert">{error}</div> : null}
    {!loadingModels && !models.length ? <div className="officialEmpty modelLabEmpty"><Bot size={30} /><b>当前项目暂无可测试模型</b><p>请先在控制台为项目绑定可用渠道并启用模型。</p></div> : null}
    ...
    {models.length ? <div className="modelLabWorkbench">
      ...
      <aside className={`modelLabResult ${result ? "hasResult" : ""}`} aria-live="polite">
        ...
        {result ? <>
          ...
          {result.modality === "image" ? image ? <figure className="modelLabImageResult"><img src={image} alt="模型生成结果" referrerPolicy="no-referrer" /><figcaption>图片生成成功</figcaption></figure> : <div className="modelLabTextResult"><ImageIcon size={18} /><p>请求成功，但响应中没有可显示的图片。</p></div> : null}
          {result.modality === "video" ? <div className="modelLabVideoResult"><Video size={22} /><div><b>{videoStatus || "任务已创建"}</b><code>{videoId || "未返回任务 ID"}</code></div>{videoId && ACTIVE_VIDEO_STATUSES.has(videoStatus.toLowerCase()) ? <button type="button" className="secondaryButton" disabled={refreshing} onClick={() => void refreshVideo()}>{refreshing ? <LoaderCircle size={14} className="spin" /> : <RefreshCw size={14} />}刷新状态</button> : null}</div> : null}
          {usage.length ? <dl className="modelLabUsage">{usage.map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value.toLocaleString("zh-CN")}</dd></div>)}</dl> : null}
          {result.requestId ? <div className="modelLabRequestId"><span>Request ID</span><code>{result.requestId}</code></div> : null}
        </> : null}
      </aside>
    </div> : null}
  </section>;
