...
在现有 FastAPI、SQLite、业务 API Key、项目隔离和管理员认证体系上增加多供应商模型中转能力。客户继续使用 `vap_live_*`，管理员可以为客户项目配置供应商渠道，再为每枚业务 Key 显式授权模型，无需客户接触真实供应商凭证。
...
- `image2.0`：OpenAI 图片模型别名，真实上游模型 ID 由管理员配置。
...
| Ability | 模型目录、项目模型绑定、Key 模型权限 |
| Distributor | 项目、模型、权限、渠道的确定性路由器 |
...
新增幂等 SQLite 表：供应商渠道、版本化凭证、模型目录、项目模型绑定、Key 模型权限、推理任务和推理用量。
...
- `/v1/models` 只返回当前业务 Key 已授权、项目已绑定且渠道可用的模型。
...
管理接口提供渠道管理、渠道测试、凭证轮换、模型目录、项目模型绑定、Key 模型权限、推理任务和用量查询。所有写操作保留现有 Session、CSRF、角色和审计规则。
...
- 现有业务 Key 自动保留素材和 `/api/video/*` 权限，但不会自动获得任何 `/v1/*` 模型权限。
- 真实上游模型 ID 不写死；只有管理员完成渠道、模型绑定和 Key 授权后才可调用。
